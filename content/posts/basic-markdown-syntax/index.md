...
lastmod: 2021-09-16T16:30:50+05:30
draft: false
author: "Abilash S"
...
Markdown is a simple way to make writing easier for the internet. To stage it simple, markdown is just regular text with a few non-alphabetic characters that holds special meaning.

{{< admonition >}}
:(far fa-bookmark fa-fw): Bookmark this page for future reference!
{{< /admonition >}}

## Headings

Heading from `h2` through `h6` are constructed with a `#` for each level:
...
This replicates the following HTML tags:
...
The HTML `<hr` element is for creating a <span style='background-color: #D4ECDD; color: #152D35'>thematic break </span> between paragraph-level elements. In Markdown, you can create a `<hr>` with any of the following:
...
A **line breaker** can be done with one blank line.


{{< admonition tip "Use HTML in Markdown" >}}
If you need a certain HTML tag(with a class & styles) you can simply use HTML.
{{< /admonition >}}
...
Wrap inline snippets of code iwth ```.
...
```html
<p>
  In this example, <code>&gt;&lt;/section&gt;</code> should be wrapped with <strong>code</strong>.
</p>
```
...
Adding a colon on the right-side / left-side of the dashes below any heading will right-align / left-align text for that column.
